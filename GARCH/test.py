...
class AdaptiveKDE:
    def __init__(self, centers, weights, bw, fs, a):
        ...
        self.cov = np.cov(centers.T, fweights=weights)
    ...
    def pdf(self, samples):
        ...
        for i, loc in enumerate(self.centers):
            density += self.weights[i] * mvnorm.pdf(x=samples, mean=loc,
                                                    cov=(self.factor ** 2) * self.hs[i] * self.cov)
    ...
    def rvs(self, size):
        ...
        for i, loc in enumerate(self.centers):
            samples[cum_sizes[i]:cum_sizes[i + 1], :] = mvnorm.rvs(size=sizes[i], mean=loc,
                                                                   cov=(self.factor ** 2) * self.hs[i] * self.cov)
...
class MLE:
    ...
    def clustering(self, seed=0, auto=False, num=2, write=False):
        if auto:
        ...
        if self.show:
            ...
            df['type'] = self.labels
            if write:
                df.to_csv('garch.csv', index=False)
    ...
    def nonparametric_estimation(self, rate, bw=1, adapt=True):
        a = 1 / self.centers.shape[1] if adapt else 0
        kdes = []
        covs = []
        Hs = []
        for i, labels in enumerate(self.__groups()):
            kdes.append(AdaptiveKDE(self.centers[labels], self.weights[labels], bw=bw, fs=self.fs[labels], a=a))
            covs.append((kdes[-1].factor ** 2) * kdes[-1].cov)
            Hs.append(np.sqrt(kdes[-1].hs))
            self.disp('KDE {}: {} ({:.4f}, {:.0f})'.format(i + 1, np.round(np.sqrt(np.diag(covs[-1])), 2),
                                                           kdes[-1].factor, kdes[-1].neff))

        if self.show:
            color = sb.color_palette()
            for i, hs in enumerate(Hs):
                sb.histplot(hs, color=color[i], label=i)

            plt.legend()
            plt.title('Adaptive bandwidth')
            plt.show()

        nonpar_proposal = lambda x: np.sum([p * kde.pdf(x) for p, kde in zip(self.proportions, kdes)], axis=0)

        def nonpar_sampler(size):
            sizes = np.round(size * np.array(self.proportions)).astype(np.int64)
        ...
        def controls(x):
            ...
            for j, loc in enumerate(self.centers[1:]):
                label = self.labels[j]
                cov = (self.fs[j] / kdes[label].gm) ** (-2 * a) * covs[label]
...
def experiment(pars, size, bw):
    ...
    mle.disp('==NIS================================================NIS==')
    mle.clustering(auto=False, num=2, write=False)
    mle.nonparametric_estimation(rate=0.9, bw=bw, adapt=True)
    mle.disp('==RIS================================================RIS==')
...
def main(save=False):
    begin = dt.now()
    Cache = [experiment((1, 0.05), size=1500, bw=1.1)]
