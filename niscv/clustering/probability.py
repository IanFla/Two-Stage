...
class Probability(Expectation):
    ...
    def density_estimation(self, bw=1.0, auto=False, factor='scott', local=False, gamma=0.3, df=0, alpha0=0.1, k=None):
        self.kde = KDE2(self.centers, self.weights_kn, labels=self.fun(self.centers), bw=bw,
                        auto=auto, factor=factor, local=local, gamma=gamma, df=df, k=k)
...
def experiment(dim, b, size_est, show, size_kn, ratio, auto=False, k=None):
    results = []
    mean = np.zeros(dim)
...
def main():
    ...
    for i in range(100):
        print(i + 1)
        result = experiment(dim=4, b=2, size_est=10000, show=False, size_kn=300, ratio=20, auto=True, k=2)
        # result = experiment(dim=4, b=2, size_est=10000, show=False, size_kn=300, ratio=20, auto=False, k=None)
